...
HvRawFilter::HvRawFilter(int all_gain, double min_down_frq, double max_up_frq, bool type_lhsh_lhpf)
{
    ...
    srate = 48000.0;//2.76.7=48000.0 error->if=0.0 out=inf fiktivno posle se smenia ot vzavisimost ot GenMessage::testwave() set_rate(double)
    A = pow(10.0,0.0/40.0);//dbGine = 0 za 0 db (0/40)
    beta = sqrt(A + A);
    ...
    dbGain1(-12.5);//dbGain1((double)(0 - 50)/4.0);//45hz
    //dbGain2((double)(0 - 50)/4.0);//45hz
    dbGain10(-12.5);//dbGain10((double)(0 - 50)/4.0);//13000hz
    //dbGain11((double)(0 - 50)/4.0);//13000hz
    AlldbGain((double)(all_gain - 50)/4.0);//AlldbGain11((double)(45 - 50)/4);//gine 45 zaradi windows
...
{
    srate = rate;
    dbGain1(-12.5);//dbGain1((double)(0 - 50)/4.0);
    //dbGain2((double)(0 - 50)/4.0);
    dbGain10(-12.5);//dbGain10((double)(0 - 50)/4.0);
    //dbGain11((double)(0 - 50)/4.0);
}
...
{
    A = pow(10.0,val/40.0);
    beta = sqrt(A + A); //qDebug()<<"val="<<val; 
    omega = 2.0 * M_PI_HV * freq1 / srate; //qDebug()<<"dbGain1"<<freq1<<srate<<omega;
    sn = sin(omega);
    cs = cos(omega);
    if (TYPE_LHSH_LHPF)
    {
    ...
    eq.f1a3_l = a1 /a0;
    eq.f1a4_l = a2 /a0;
    eq.f1x1_l = eq.f1x2_l = 0.0;
    eq.f1y1_l = eq.f1y2_l = 0.0;
}
/*
...
void HvRawFilter::dbGain10(double val)
{
    A = pow(10.0,val/40.0);
    beta = sqrt(A + A);
    omega = 2.0 * M_PI_HV * freq10 / srate; //qDebug()<<"dbGain10====="<<freq10<<srate<<omega;
    sn = sin(omega);
    cs = cos(omega);
    if (TYPE_LHSH_LHPF)
    {
    ...
    eq.f10a3_l = a1 /a0;
    eq.f10a4_l = a2 /a0;
    eq.f10x1_l = eq.f10x2_l = 0.0;
    eq.f10y1_l = eq.f10y2_l = 0.0;
}
/*
...
{
    Gain_all = pow(10.0,val/20.0);	// +- 12.5db //qDebug()<<Gain_all;//49=0.971628
}
